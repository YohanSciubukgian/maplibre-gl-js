...
2. Download https://imagemagick.org/archive/binaries/magick from https://imagemagick.org/script/download.php#linux
   Install Fuse if needed, using the command `sudo apt-get install fuse libfuse2`
3. Run the following command `for file in *.{jpg,png}; do ./magick "$file" -quality 90 "${file%.*}.webp"; done`
4. Commit the image.
